...
#
#
#     def forward(self, image, tuning_prompt):
...
#         text_prompt1 = ['small']
#         text_prompt1 = tokenizer(text_prompt1).to(device)
#         text_prompt1 = model.encoder_tuning(text_prompt1)
#
#         return text_prompt1.detach()
#
#     tuning_prompt = get_tuning_prompt()
#
#     # v1
#     class_name = ["normal object", "abnormal object"]
#     # v2
#     # class_name = ["normal object", "damaged object"]
#
#     clip_model = CustomCLIP(class_name, model, model_configs, device)
#
#     state_dict = torch.load('D:\Cyh\VAND-APRIL-GAN-cyh\model_dict/epoch_14.pth', map_location=device)
#     ##################################################################
#     # state_dict = state_dict["model_state_dict"]  # test
#     state_dict = state_dict["trainable_linearlayer"]
#     original_state_dict = clip_model.state_dict()
#     original_state_dict.update(state_dict)
#     clip_model.load_state_dict(original_state_dict)
#
#
#     results = {}
#     results['cls_names'] = []
...
#
#         text_probs, prompt, anomaly_maps = clip_model(image, tuning_prompt)
...
from cyhModule.model import LinearLayer
...
def test(args):
    ...
    # #2
    # text_prompt = ["flawless object",
    #                "flawed object", ]

    # #3
    # text_prompt = ["undefective object",
    #                "defective object", ]

    text_prompt = tokenizer(text_prompt).to(device)
    class_embeddings = model.encode_text(text_prompt)
    ...
    for items in test_dataloader:
        ...
        #     text_features = torch.stack(text_features, dim=0)
            ...
            # sample
            text_probs = (100.0 * image_features @ class_embeddings.t()).softmax(dim=-1)
            results['pr_sp'].append(text_probs[0][1].cpu().item())
            ...
            for layer in range(len(patch_tokens)):
                ...
                anomaly_map = (100.0 * patch_tokens[layer] @ class_embeddings.t())
    ...
    for obj in obj_list:
        ...
        tmp_pr_sp = np.isnan(pr_sp)
        tem_bool_nan = np.any(tmp_pr_sp)
        if tem_bool_nan == True:
            nan_index = np.where(tmp_pr_sp == True)
            pr_sp = np.delete(pr_sp, nan_index[0])
            gt_sp = np.delete(gt_sp, nan_index[0])
