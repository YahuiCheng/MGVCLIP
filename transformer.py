...
import numpy as np
from multi_scale_convolution_adapter import Multi_scale_adapter
...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        if self.is_visual:
            ...
            N, L, D = x.shape
            iw = int(np.sqrt(L - 1))
            cls_token, feature_map = x[:, 0:1, :], x[:, 1:, :]
            residual = feature_map
            feature_map = feature_map.permute(0, 2, 1).view(N, -1, iw, iw)
            feature_map = multi_scale_adpter(feature_map)
            feature_map = feature_map.view(N, D, -1).permute(0, 2, 1)  #
            output = residual + feature_map
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.image_size = 240
        if self.is_visual:
            self.multi_scale_adpter = nn.ModuleList([
                # vb16+
                Multi_scale_adapter(896, 896) for idx in range(12)
            ])
        ...
        self.resblocks = nn.ModuleList([
            ResidualAttentionBlock(
                ...
                idx=idx, is_visual=self.is_visual)
    ...
    def forward(self, x: torch.Tensor, out_layers: list = [3, 6, 9],
        ...
        for r in self.resblocks:
            ...
            if self.grad_checkpointing and not torch.jit.is_scripting():
                ...
                if self.is_visual:
                    if idx == 12:
                        x, attn = r(x, self.multi_scale_adpter[multi_scale_layer_idx], attn_mask=attn_mask)
                        out_attn.append(attn)
                    else:
                        x, attn_tmp = r(x,self.multi_scale_adpter[multi_scale_layer_idx], attn_mask=attn_mask)

                    if idx in out_layers:
                        out_tokens.append(x)
                        # out_tokens = x
                    multi_scale_layer_idx += 1
                else:
                    if idx == 12:
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, out_layers: list):
        ...
        B, C, L = attn[0].shape
        H = int(np.sqrt(L-1))  # 减1是因为多加了一个token
        # 没cuda运行不了
