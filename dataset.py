...
class VisaDataset(data.Dataset):
	...
	def __getitem__(self, index):
		...
		# img = cv2.imread(os.path.join(self.root, img_path), cv2.IMREAD_COLOR)
		...
		## img = self.transform(Image.fromarray(img)) if self.transform is not None else img
...
class MVTecDataset(data.Dataset):
	...
	def __getitem__(self, index):
		...
		if random_number < self.aug_rate:
			...
			if anomaly == 0:
				img_mask = Image.fromarray(np.zeros((img.size[0], img.size[1])), mode='L')
				## img_mask = Image.fromarray(np.zeros((240, 240)), mode='L')
